...
\begin{abstract}
\philipp{I've reordered this quite a bit
I won't rework the introduction as much but make sure you spend some time making 
clear why: heap corruption is a problem, why we need tooling for it, why we need tooling 
for Scudo and then how we solve the problem and what our main results are
}
...
\philipp{don't use passive voice, just use we}
...
Android native libraries running on Android 11.\philipp{ANdroid version of Testphone}
...
\philipp{Add some sentences here that follow the structure of the abstract. 1: 
Heap bugs important. 2: Need dynamic debugging for the heap to understand these bugs. 
3: Android uses new and mostly unknown allocator (Scudo, make sure it's spelled 
consistently)
And then you can add the text below}
\philipp{Give a concrete example that heap bugs in Android apps are dangerous and 
may be exploited: https://www.exploit-db.com/docs/48632}
\philipp{When first talking about Scudo add a reference to the LLVM Scudo website}
...
We evaluate our tool by triaging \philipp{XX} heap-related crashes on a real Android 
device.
...
\philipp{you can probably remove this   and focus on improving the scudo internals section}
...
hardened allocator.\philipp{Reference to the LLVM Scudo page}
...
\philipp{Add a figure that shows the detailed structure of the header}
...
\philipp{when you talk about metadata please not where that metadata ends up 
being stored}
...
class, that works differently than the other classes. \philipp{that's used for 
the secondary chunk?, I would just say that the primary class ids start from 1}
...
\philipp{Figure to illustrate primary chunk memory regions and the classes on 
Android}
...
\philipp{This section would be greatly improved with figures, here I would add 
an image to illustrate how the free list in the primary allocator works}
...
\philipp{*optional* talk about quarantine}
...
Linux systems, which is included in the gef plugin \philipp{cite} for gdb. The scudo plugin
...
\philipp{Add a sentence here explaining what you want to show when evaluating 
your tool.}

\section{Real Device}
\philipp{Find better title}
...
\end{verbatim}
\philipp{I would omit the WSL specific stuff}
...
\philipp{Enclose the code in a figure}
...
\philipp{show conretely how you use your plugin's commands to analyze the crashes, 
alsow with screenshots}
...
\philipp{since we're running a bit short on time I would not invest any work here 
and just cut this chapter, focus on making the other chapters better.
}
...
\philipp{Add a sentence like: In this project we implemented a plugin ... }
...
about scudo, there was no easy way to investigate crashes \philipp{dynamically \& on real phone} 
related to scudo on
