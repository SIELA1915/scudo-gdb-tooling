        ...
        try:
            ...
            if "Cannot look up value of a typedef" in str(e):
                ...
                self.class_size_list = ([-1, 0x00020, 0x00030, 0x00040, 0x00050,
                                         ...
                                         0x07310, 0x08210, 0x10010, 0x18010, 0x20010,
                                         0x28010, 0x30010, 0x38010, 0x40010,] if bit64 else
                                        [-1, 0x00020, 0x00030, 0x00040, 0x00050, 0x00060,
                ...
                self.num_classes = len(self.class_size_list)
                self.largest_class_id = self.num_classes - 1
                self.batch_class_id = 0
...
def search_for_scudo_per_class_array(thread_id) -> int:
    ...
    try:
        ...
        stats_addr = parse_address("*(((void**)&Allocator)+9)")
...
def search_for_scudo_large_block() -> int:
    ...
    try:
        ...
        addr = parse_address("*(*(((void***)&Allocator)+9)-6)")
...
class ScudoTransferBatch:
    ...
    def transfer_batch_t() -> Type[ctypes.Structure]:
        ...
        fields = [
            ...
            ("batches", config.max_num_cached_hint*pointer),
        ]
...
class ScudoPerClass:
    ...
    def per_class_t() -> Type[ctypes.Structure]:
        ...
        fields = [
            ...
            ("class_size", pointer),
            ("chunks", 2*config.max_num_cached_hint*pointer),
        ]
...
class ScudoLargeBlock:
    ...
    def block_end(self) -> int:
        return self.__large_block.block_end
    ...
    def __str_extended(self) -> str:
        ...
        msg.append("Block end: {0:#x}".format(self.block_end))
...
class ScudoHeapCommand(GenericCommand):
    ...
    _cmdline_ = "scudo"
    _syntax_  = f"{_cmdline_} (chunk|regions|region|transferbatch|perclass|largeblock)"
...
class ScudoBatchGroupCommand(GenericCommand):
    ...
    @only_if_gdb_running
    def do_invoke(self, _: List[str], **kwargs: Any) -> None:
        err("Not supported in this version of scudo")
        return
        args : argparse.Namespace = kwargs["arguments"]
        if not args.address:
